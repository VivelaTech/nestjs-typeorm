...
let TypeOrmModule = TypeOrmModule_1 = class TypeOrmModule {
    ...
    static forCustomRepository(repositories, dataSourceName) {
        const providers = [];
        for (const repository of repositories) {
            const entity = Reflect.getMetadata(common_2.TYPEORM_CUSTOM_REPOSITORY, repository);
            if (!entity) {
                continue;
            }
            providers.push({
                inject: [(0, common_2.getDataSourceToken)(dataSourceName)],
                provide: repository,
                useFactory: (dataSource) => {
                    const baseRepository = dataSource.getRepository(entity);
                    const customRepository = new repository(baseRepository.target, baseRepository.manager, baseRepository.queryRunner);
